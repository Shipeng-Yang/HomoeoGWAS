"""Polyploid genomic prediction (GBLUP) with subgenome-stratified GRMs.

Three model tiers compared side by side:
    ...
    Tier 2 (multi-K + homoeolog kernel):
        ...
        Auto-fallback: if σ²_h hits the boundary or kernel cond > 1e4, drop
        K_hom and record drop_reason in the result.

Closed-form BLUP prediction. Given REML estimates (σ²_j, σ²_e, β) fit on the
training fold:
...
This is the standard Henderson mixed-model BLUP formula for multi-kernel
models; we Cholesky-factor V_tt once per fold and solve.

``run_cv_gblup`` runs n_repeats x n_folds stratified-by-trait-quantile GBLUP
fits and aggregates per-fold Pearson r2 + top-10% enrichment + paired-bootstrap
95% CI over individuals.
"""
...
class FoldResult:
    ...
    n_test: int
    r_pearson: float
    r2_pearson: float          # r_pearson**2 (signed-r squared, not 1 - SSE/SST)
    rmse: float
...
class TierSummary:
    ...
    ci_r2_low: float                   # paired-bootstrap 95% CI (vs Tier 0)
    ci_r2_high: float
    mean_r: float
    mean_top10_enrichment: float
    n_dropped_kernels: int             # folds where K_hom was dropped
    drop_reasons: dict[str, int]       # count per drop reason
...
def stratified_folds(y: np.ndarray, n_folds: int = 5, n_strata: int = 5,
    ...
    """Return a list of test-index arrays, each fold balanced by trait quantile.

    Rank y into n_strata bins, then within each stratum assign individuals to
    folds at random. Keeps every fold's trait distribution close to the panel
    mean, which matters for small panels (e.g. n=297 rapeseed) where a single
    random split can over-sample one tail.
    """
    if rng is None:
        rng = np.random.default_rng()
    n = len(y)
    n_strata = min(n_strata, n_folds)
    # quantile-based strata -> equal counts per stratum
...
def blup_predict(
    ...
    # alpha = V_tt^{-1} resid via two triangular solves
    z = np.linalg.solve(L, resid)
    alpha = np.linalg.solve(L.T, z)

    # g_test = Σ_j σ²_j K_j[test, train] alpha
...
def top_k_enrichment(y_obs: np.ndarray, y_pred: np.ndarray, k_pct: float = 0.10) -> float:
    ...
    Rank by predicted value descending, pick the top k_pct. Hit rate = fraction
    of the true top-k_pct phenotypes captured by that selection; enrichment is
    hit_rate / k_pct, with >1 meaning better than random."""
    n = len(y_obs)
    if n == 0:
        return float("nan")
...
def paired_bootstrap_ci(
    ...
    """Block-aware paired bootstrap over folds for delta-r2 vs baseline."""
    ...
    *,
    ...
    tiers
        ``tier0``  : single pooled GRM. Construction priority:
                     1) ``k_pool`` if provided (a pre-built all-SNP GRM, the
                        proper rrBLUP baseline);
                     2) SNP-count-weighted average of ``per_subgenome_grms``
                        using ``snp_counts={sg: m_j}`` (equivalent to building
                        K from concatenated dosage, up to VanRaden constants);
                     3) fallback: equal-weight average across subgenomes with a
                        warning. This is unfair on unbalanced panels (e.g. the
                        strawberry iStraw90 array, sub A 14.6K SNPs vs sub D
                        0.4K), since it gives under-represented subgenomes equal
                        voice in the baseline and inflates the tier1 - tier0
                        delta-r2. Pass ``k_pool`` or ``snp_counts`` for
    ...
    # build the three kernel sets once (full n x n)
    ...
    # paired bootstrap CI for delta-r2 vs tier0
